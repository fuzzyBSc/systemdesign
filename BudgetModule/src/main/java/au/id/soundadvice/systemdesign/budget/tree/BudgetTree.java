 ...
 */
public class BudgetTree implements Tree {
    ...
    public static final class LogicalTreeNode implements TreeNode {
        ...
        public LogicalTreeNode(Optional<Record> function, BaselinePair.Selector selector, SortedMap<String, TreeNode> children) {
        ...
        @Override
        public RecordID getIdentifier() {
            return function.map(Record::getIdentifier).orElse("");
        }
    ...
    public BudgetTree(BaselinePair baselines) {
        Optional<Record> systemOfInterest = Identity.getSystemOfInterest(baselines);

        Map<String, Record> parentFunctions;
        if (systemOfInterest.isPresent()) {
            parentFunctions = BudgetAllocation.findOwnedFunctions(baselines.getParent(), systemOfInterest.get())
                    .collect(Collectors.toMap(Identifiable::getIdentifier, t -> t));
        } else {
            parentFunctions = Collections.emptyMap();
        }

        Map<Optional<Record>, Map<String, Record>> childAllocations
                = BudgetAllocation.find(baselines.getChild())
                .collect(Collectors.groupingBy(
                        child -> child.getTrace().flatMap(
                                trace -> Optional.ofNullable(parentFunctions.get(trace))),
                        Collectors.toMap(Identifiable::getIdentifier, t -> t)));

        this.allocation = childAllocations.entrySet().stream()
                .filter(entry -> entry.getKey().isPresent())
                .map(entry -> {
                    SortedMap<String, TreeNode> children = Optional.ofNullable(childAllocations.get(entry.getKey()))
                            .orElse(Collections.emptySortedMap())
                            .entrySet().stream()
                            .collect(Collectors.toMap(
                                    childEntry -> childEntry.getValue().getLongName(),
                                    childEntry -> new LogicalTreeNode(Optional.of(childEntry.getValue()), BaselinePair.Selector.CHILD, Collections.emptySortedMap()),
                                    (u, v) -> {
        ...
        this.orphans = new LogicalTreeNode(
                Optional.empty(),
                BaselinePair.Selector.PARENT,
                Optional.ofNullable(childAllocations.get(Optional.<Record>empty()))
                .orElse(Collections.emptySortedMap())
                .entrySet().stream()
                .collect(Collectors.toMap(
                        childEntry -> childEntry.getValue().getLongName(),
