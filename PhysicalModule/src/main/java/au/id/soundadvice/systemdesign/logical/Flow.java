...
public enum Flow implements RecordType {
    ...
    static Pair<BaselinePair, Record> addWithGuessedType(BaselinePair baselines, String now, RecordConnectionScope scope) {
        Pair<BaselinePair, Record> tmp = FlowType.define(baselines, now, guessTypeName(baselines, scope));
        baselines = tmp.getKey();
        Record flowType = tmp.getValue();
        return add(baselines, now, scope, flowType);
    }

    private static String guessTypeName(BaselinePair baselines, RecordConnectionScope newChildScope) {
    ...
    private static Optional<Record> getExpectedTrace(
            BaselinePair baselines, RecordConnectionScope childScope, Record flowType) {
    ...
    @CheckReturnValue
    public static Pair<BaselinePair, Record> add(
            final BaselinePair baselines, String now,
        ...
        if ( // At least one end must be internal
            ...
            Record iface;
            {
                ...
                Pair<BaselinePair, Record> tmp = Interface.iface.connect(baselines, now, interfaceScope);
                result = tmp.getKey();
                iface = tmp.getValue();
            }
    ...
    private BaselinePair addToParent(BaselinePair baselines, String now, RecordConnectionScope parentScope, Record parentFlowType) {
        // Use a fake baseline pair to modify the parent baseline as if it were the child
        BaselinePair fakeBaselinePair = new BaselinePair(
    ...
    private BaselinePair removeFromParent(BaselinePair baselines, String now, RecordConnectionScope parentScope, Record parentFlowType) {
        return baselines.setParent(remove(baselines.getParent(), now, parentScope, parentFlowType));
    }

    private BaselinePair removeFromChild(BaselinePair baselines, String now, RecordConnectionScope parentScope, Record parentFlowType) {
    ...
    public Stream<Problem> getUntracedParentProblems(BaselinePair context, Stream<Record> untracedParents) {
        Map<String, Record> flowedDownFunctions = Function.find(context.getChild())
    ...
    private BaselinePair fixTrace(BaselinePair baselines, String now, Record record) {
