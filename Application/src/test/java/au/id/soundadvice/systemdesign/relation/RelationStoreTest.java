...
public class RelationStoreTest {
    ...
    public void testValueOf() {
        ...
        FromRelation typeMismatch = new FromRelation(from.getIdentifier());
        FromRelation dangling = new FromRelation(UUID.randomUUID().toString());
        ...
        List<? extends FromRelation> reverse = store.findReverse(
                to.getIdentifier(), FromRelation.class)
                .collect(Collectors.toList());
        assertEquals(1, reverse.size());
        assertSame(from, reverse.iterator().next());
        List<ToRelation> toByClass = store.findByClass(ToRelation.class)
                .collect(Collectors.toList());
        assertEquals(1, toByClass.size());
        assertSame(to, toByClass.iterator().next());
        List<FromRelation> fromByClass = store.findByClass(FromRelation.class)
                .collect(Collectors.toList());
